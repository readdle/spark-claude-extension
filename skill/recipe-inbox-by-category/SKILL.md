...
- For each category, only call `thread` for a specific `<id>` when the user asks about a specific email.
- Combine with `events` set to `today` for a full morning briefing.
