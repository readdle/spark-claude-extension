...
// macOS: use the absolute path. GUI apps inherit a minimal PATH that does
// not include /usr/local/bin, where Spark CLI Setup installs the `spark`
// symlink, so resolving by name would fail under Claude Desktop.
// Windows: there is no symlink; Spark CLI Setup adds the bundle directory
// to the user PATH, so resolve `spark.exe` by name (Claude Desktop inherits
// the updated PATH once restarted). Override either with SPARK_PATH.
...
const ATTACHMENT_MAX_BYTES = Number(process.env.SPARK_ATTACHMENT_MAX_BYTES) || 50 * 1024 * 1024;
...
function defaultAnnotationsFor(toolName) {
    return READ_ONLY_TOOLS.has(toolName) ? { readOnlyHint: true } : { destructiveHint: true };
}
...
async function checkSparkVersion() {
    try {
        const { stdout } = await execFileAsync(SPARK_PATH, ["--version"], {
            timeout: 5_000
        });
        ...
        console.error(`[spark-mcp] WARNING: could not check spark version: ${err.message}`);
    }
}
...
async function loadCatalogOnce() {
    try {
        ...
        const { stdout, stderr } = await execFileAsync(SPARK_PATH, ["tools"], {
            timeout: 10_000
        });
        ...
        console.error(`[spark-mcp] loaded ${catalog.tools.length} tools from spark`);
    } catch (err) {
        const detail =
            err.code === "ENOENT"
                ? `'${SPARK_PATH}' not found. Install Spark Desktop and enable the Spark CLI (Settings → AI Agents → Spark CLI Setup). On Windows, restart Claude Desktop afterwards so it inherits the updated PATH. Or set SPARK_PATH to the spark binary.`
                : err.stderr?.trim() || err.message;
        catalogError = new Error(`Cannot load spark tools: ${detail}`);
...
server.setRequestHandler(CallToolRequestSchema, async request => {
    ...
    // recipients, subjects, or bodies - so the server log stays diagnostic
    // without leaking message content.
    console.error(
        `[spark-mcp] tool '${name}' -> spark ${toolDef.command} (${args.length - 1} args)`
    );
    ...
    try {
        ...
        console.error(`[spark-mcp] tool '${name}' failed: ${message}`);
        return {
...
async function handleAttachment(input) {
    ...
    if (!output) {
        return {
            content: [{ type: "text", text: "Error: spark CLI returned no output." }],
            isError: true
        };
    }
    ...
    // (URLs, MIME parameters), so we split on the first `: ` only. Split on
    // \r?\n so the Windows CLI's CRLF output doesn't leave a trailing \r on
    // every value (e.g. a MIME type of "image/png\r").
    const parsed = {};
    ...
    try {
        ...
        const message = err.stderr?.toString("utf8").trim() || err.message;
        return {
            content: [{ type: "text", text: `Error: ${message}` }],
            isError: true
        };
